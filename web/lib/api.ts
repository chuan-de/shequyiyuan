import { API_ROUTES, type ApiErrorResponse, type ApiResponse, type AuthResponse, type CurrentUserResponse, type DictionaryItemResponse, type DictionaryResponse, type EntityRecord, errorCodeMessages, type HealthResponse, type LoginPayload, type RegisterPayload } from './api-contract';
...
export async function changeEntityStatus(token: string, route: string, id: number, enabled: boolean): Promise<void> { await unwrapResponse(await apiFetch(`${route}/${id}/status`,{method:'PATCH',headers:{...authHeader(token),'Content-Type':'application/json'},body:JSON.stringify({ enabled })})); }
