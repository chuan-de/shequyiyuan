...
| AI 基础设施（ai-core） | — | Not Started | 计划：`docs/ai-features-plan.md`（Phase 0）；DB：V37（权限）、V38（`ai_audit_log`）；代码：`com.hospital.ai.*`（待建）；模型：Doubao via OpenAI 兼容协议 | 2026-05-24 | 待指定 |
| AI 病历识别（ai-vision） | — | Not Started | 计划：`docs/ai-features-plan.md`（Phase 1）；DB：V39（`medical_record.ai_extracted` + `ai_extraction_history`）；接口：`POST /api/v1/ai/vision/parse-medical-record`；模型：`doubao-seed-2.0-lite` | 2026-05-24 | 待指定 |
| AI 患者 RAG（ai-patient-rag） | — | Not Started | 计划：`docs/ai-features-plan.md`（Phase 2）；DB：V40（pgvector ext）、V41（`patient_knowledge_chunk`）、V42（`patient_profile.ai_consent_at` + `ai_dead_letter`）；接口：`POST /api/v1/ai/patient/{patientId}/ask`；模型：`doubao-embedding-text-240715` + `doubao-seed-2.0-lite` | 2026-05-24 | 待指定 |
| AI 社区问诊（ai-consult） | — | Not Started | 计划：`docs/ai-features-plan.md`（Phase 3）；DB：V43（`ai_consult_session` + `ai_consult_message`）；接口：`POST /api/v1/ai/consult/*`（SSE 流式）；页面：`web/app/ai-consult/page.tsx`；模型：`doubao-seed-2.0-lite` | 2026-05-24 | 待指定 |
